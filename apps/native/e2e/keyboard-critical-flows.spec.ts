...
test.describe('🚨 CRITICAL: Real Browser Keyboard Tests', () => {
  ...
  test.describe('🎯 Modal Operations (Critical)', () => {
    test('should open and close help modal with keyboard', async ({ page }) => {
      ...
      // Open help with Cmd+? (or Ctrl+? on non-Mac)
      const isMac = process.platform === 'darwin';
      if (isMac) {
        await page.keyboard.press('Meta+Shift+Slash'); // Cmd+?
      } else {
        await page.keyboard.press('Control+Shift+Slash'); // Ctrl+?
      }
  ...
  test.describe('🎯 Real User Workflows', () => {
    test('should handle complete user journey: browse → help → create → cancel', async ({ page }) => {
      const isMac = process.platform === 'darwin';
